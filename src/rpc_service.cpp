...
void HttpServiceImpl::GetSecuritySnapshots(google::protobuf::RpcController* cntl_base, const rpc_server::HttpRequest* req, rpc_server::HttpResponse* res, google::protobuf::Closure* done) {
...
void HttpServiceImpl::GetSecurityList(google::protobuf::RpcController* cntl_base, const rpc_server::HttpRequest* req, rpc_server::HttpResponse* res, google::protobuf::Closure* done) {
    brpc::ClosureGuard done_guard(done);
    ...
    for (int i = 0; i < (count / 1000) + 1; ++i) {
        ...
        std::copy_if(security_infos.begin(), security_infos.end(), std::back_inserter(filtered_infos), [&](const SecurityInfo& info) {
            if (market == Market::kSH) {
                ...
                return info.code.size() > 1 && (0 == strncmp(info.code.c_str(), "00", 2) || 0 == strncmp(info.code.c_str(), "30", 2));
            }
        });
...
void HttpServiceImpl::GetSecurityKlines(google::protobuf::RpcController* cntl_base, const rpc_server::HttpRequest* req, rpc_server::HttpResponse* res, google::protobuf::Closure* done) {
    brpc::ClosureGuard done_guard(done);
